...
export default function CarWraps() {
  ...
  useEffect(() => {
    ...
    if (savedPosts) {
      ...
      setPosts(allPosts.filter(post => post.page === 'carwraps'));
    }

    const savedAvailability = localStorage.getItem('availabilityByService');
    if (savedAvailability) {
      const allAvailability = JSON.parse(savedAvailability);
      setAvailability((allAvailability.carwraps || []).filter(slot => new Date(slot.date) >= new Date()));
    }
  }, []);
  ...
  const handleBookingSubmit = (e) => {
    ...
    if (allAvailability.carwraps) {
      const slotIndex = allAvailability.carwraps.findIndex(s => s.id === selectedSlot.id);
      if (slotIndex !== -1) {
        allAvailability.carwraps[slotIndex].status = 'booked';
  ...
  return (
    <div className="container">
      ...
      <div style={{
        ...
        <h2 style={{ color: '#f50505', marginBottom: '1rem' }}>Book Car Wrap Service</h2>
        ...
        <form onSubmit={handleBookingSubmit}>
          ...
          <textarea
            placeholder="Tell us about your car wrap needs (vehicle type, design preferences, etc.)..."
